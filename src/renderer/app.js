...
const state = {
  ...
  autoSaveTimers: {},
  autoSaveEnabled: false,   // OFF by default; user enables via File ▸ Auto Save
  theme: 'apex-midnight',
  welcomeVisible: true,
...
/**
 * Apply the auto-save preference. `persist` writes it to localStorage (used when
 * the user toggles it); startup passes persist=false. Always mirrors the value
 * back to the native File ▸ Auto Save checkbox.
 */
function applyAutoSaveSetting(enabled, persist) {
  state.autoSaveEnabled = !!enabled;
  if (persist) { try { localStorage.setItem('apexstudio-autosave', enabled ? '1' : '0'); } catch (_) {} }
  if (!enabled) { Object.keys(state.autoSaveTimers).forEach(clearAutoSave); }
  if (dom.statusAutosave) {
    dom.statusAutosave.title = enabled
      ? 'Auto-save is on — saves 3s after you stop typing'
      : 'Auto-save is off — press ⌘S to save';
  }
  try { window.apexStudio.notifyAutoSave && window.apexStudio.notifyAutoSave(state.autoSaveEnabled); } catch (_) {}
}
...
async function renderTreeDir(dirPath, parentEl, depth) {
  ...
  // Build all rows off-DOM, then insert once — appending each row to the live
  // tree individually forces a reflow per entry and is what makes expanding a
  // large folder stutter. A DocumentFragment collapses it to a single reflow.
...
function initIpcHandlers() {
  ...
  // Auto-save preference (default OFF). Toggled from the native File ▸ Auto Save
  // menu item; persisted in localStorage and synced back to the menu checkbox.
  api.on('settings:set-autosave', (val) => applyAutoSaveSetting(val, true));
  try { applyAutoSaveSetting(localStorage.getItem('apexstudio-autosave') === '1', false); }
  catch (_) { applyAutoSaveSetting(false, false); }
}
